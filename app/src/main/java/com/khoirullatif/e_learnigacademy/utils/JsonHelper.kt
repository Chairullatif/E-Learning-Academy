package com.khoirullatif.e_learnigacademy.utils
...
class JsonHelper(private val context: Context) {
    ...
    private fun parsingFileToString(fileName: String): String? {
        return try {
            // is merupakan Reserved Word (contoh: Res. Word adalah spt val var)
                // maka agar dapat digunakan sebagai nama variabel diperlukan tanda ``
        ...
        try {
            ...
            if (result != null) {
                ...
                for (i in 0 until listArray.length()) {
                    ...
                    val courseId = course.getString("courseId")
                    val title = course.getString("title")
                    val position = Integer.parseInt(course.getString("position"))
